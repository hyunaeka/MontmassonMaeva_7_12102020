<template>
  <div id="wall" class="wall container mt-5">
    ...
    <div class="card container p-5">
      ...
      <h1 class="pb-5 form__title">
        Publication de {{ articleUnique.userName }} 
      </h1>

        <!-- Affichage du média importer -->
      <img :src="articleUnique.attachment" class="rounded mx-auto d-block" />  
      <h2 class="m-5 d-flex justify-content-center form__title">

          <!-- Afficgafe du titre du post -->

        {{ articleUnique.title }} 
      </h2>
      <p class="mb-5 d-flex justify-content-center">
      ...
      <div
        v-if="this.idPostUser == this.idTokenAdmin"
        class="card card__modifyPost container p-5"
      >
        <div class="form-row">
          <label class="pb-3 form__title" for="title"
            >Nouveau titre du post</label
          >
          <input
            v-model="title"
            type="text"
          ...
          <label class="pt-4 pb-3 form__title" for="content"
            >Contenu du post</label
          >
          <textarea
            v-model="content"
            type="text"
            id="content"
            class="form-control content__modify"
            required
          ></textarea>
          <p class="modify">*obligatoire</p>
        </div>
        <div class="form-row pt-5 pb-5">
          ...
          <input
            ...
            accept="image/*"
          />
          <p class="modify">*Falcultatif</p>
        </div>
        <button
          class="btn__modify btn form__title"
          v-if="this.idPostUser == this.idTokenAdmin"
          @click="modifyPost(articleUnique.id) != null"
        >
          Modifier le post
        </button>
      </div>
      ...
      <div class="card__deletePost container pt-5 pb-5">
        ...
        <button
          class="btn-danger btn btn__suppr form__title"
          v-if="this.idPostUser == this.idTokenAdmin"
          @click="deletePost(articleUnique.id)">
          Supprimer l'article 
        </button>
      ...
      <div class="card card__createcomment container p-5 mb-4">
        <div class="form-row">
          ...
          <button class="mt-3 btn form__title" @click="createComment()">
      ...
      <div class="card card__comment__test d-flex">
        <div
          class="card__comment card p-5 m-5"
          :key="index"
          v-for="(datapost, index) in comments">
          <div class="d-flex flex-row-reverse">

              <!-- Suppression d'un commentaire -->

            <i
              @click="deleteComment(datapost.id)"
              class="fas btn fa-times suppr">
            </i> 
          </div>

          <!-- affichage du pseudo de l'utilisateur commentaire -->
...
/************************************************************************* Appelle fonction pour récupérer le post selectionné *******************************************************************************/
...
// Permet de vérifier le Token et donc l'identité valide d'un utilisateur sinon aucun affichage
    ...
    axios
      .get(`http://localhost:3000/api/posts/userPost/${this.id}`)
      .then((response) => {
...
/************************************************************************* Fonction créer un commentaire *******************************************************************************/
    ...
    createComment: function () {
      ...
      axios
        .post(
          `http://localhost:3000/api/comments/create/${this.id}`,
          commentSend
        )
        .then((response) => {
...
/************************************************************************* Fonction Supprimer un commentaire *******************************************************************************/
    ...
    deleteComment: async function (idComment) {
      ...
      axios
        .delete(`http://localhost:3000/api/comments/delete/` + idComment)
        .then(() => {
          location.reload();
        });
    },
...
/************************************************************************* Fonction supprimer un post *******************************************************************************/
    ...
    deletePost: async function (idpost) {
      ...
      axios
        .delete(`http://localhost:3000/api/posts/delete/` + idpost)
        .then(() => {
...
/************************************************************************* Fonction modifier un post *******************************************************************************/
    ...
    modifyPost: async function (idpost) {
      ...
      axios
        .put(
          `http://localhost:3000/api/posts/modify-post/` + idpost,
          postModifySend
        )
        .then((response) => {
