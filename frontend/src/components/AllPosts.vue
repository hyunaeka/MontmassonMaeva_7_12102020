  ...
  <div id="wall" class="wall container">
    ...
    <div>
      <div class="container d-flex justify-content-center">
        <button class="btn p-2 mb-5 mt-5">
          <router-link :to="'/createPost/'">Créer un article</router-link>
        </button>
      </div>
      ...
      <div class="container d-flex flex-column-reverse">
          ...
          <!-- affichage de tous les post (titre, date, pseudo) -->

        <div
          class="card card__post mb-2 "
          :key="index"
          v-for="(article, index) in posts"
        >
            <router-link :to="`/postsView/${article.id}`"
            ><h1 class="container post__title">{{ article.title }}</h1></router-link
          > 
          <br>
          <div class=" container d-flex flex-wrap">

          <p class="card__username ">
            By <i class="fas fa-user p-1"></i> {{ article.userName
            }}</p>
           
             <p>  <span> <i class="far fa-clock"></i>Le
              {{ article.createdAt | formatDate }}</span><i class="far fa-heart"></i></p>
...
/************************************************************************* Fonction affichage de tous les post *******************************************************************************/
  ...
  created() {
    ...
    axios.get("http://localhost:3000/api/posts/all").then((response) => {
